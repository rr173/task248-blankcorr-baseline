...
func (svc *Service) SelfCheck(ctx context.Context, batchID int64) (*SelfCheckResult, error) {
	...
	samples, err := svc.store.ListMeasurements(batchID, model.KindSample, nil)
	if err != nil {
		return nil, err
	}
	blanks, err := svc.store.ListMeasurements(batchID, model.KindBlank, nil)
	if err != nil {
		return nil, err
	}
	standards, err := svc.store.ListMeasurements(batchID, model.KindStandard, nil)
	if err != nil {
		return nil, err
	}

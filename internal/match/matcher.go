...
func (m *Matcher) Match(batchID int64, window time.Duration) ([]*model.CorrectionRelation, error) {
	...
	for _, smp := range samples {
		if NearestWithin(blanks, smp.MeasuredAt, window) == nil {
			return nil, fmt.Errorf("%w: sample %d has no eligible blank within matching window", model.ErrConflict, smp.ID)
		}
	}
	...
	for _, smp := range samples {
		...
		blank := NearestWithin(blanks, smp.MeasuredAt, window)
		c.BlankID = blank.ID

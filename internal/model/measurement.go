...
func (m MeasurementInput) Validate() error {
	...
	if math.IsNaN(m.Ratio) || math.IsInf(m.Ratio, 0) {
		return NewValidationError("ratio", m.Ratio, "isotope ratio must be finite")
	}
	if m.RatioUnc < 0 {
		return NewValidationError("ratio_unc", m.RatioUnc, "uncertainty must be >= 0")
	}
	if math.IsNaN(m.RatioUnc) || math.IsInf(m.RatioUnc, 0) {
		return NewValidationError("ratio_unc", m.RatioUnc, "uncertainty must be finite")
	}
	...
	if math.IsNaN(m.CertifiedRatio) || math.IsInf(m.CertifiedRatio, 0) {
		return NewValidationError("certified_ratio", m.CertifiedRatio, "certified ratio must be finite")
	}
